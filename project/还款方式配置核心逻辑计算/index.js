...
/**
 * 计算贷款到期日
 * @param {Date} serverTime 提款日期
 * @param {Date} endTime 额度到期日期
 * @param {number} limit 还款期数
 * @returns {string} 贷款到期日
 */
function calcDate(serverTime, endTime, limit) {
    ...
    let curTime =
        calcloanDate.getTime() - endTime.getTime() < 0 ? calcloanDate : endTime;
    let res =
        curTime.getFullYear() +
        "-" +
        (curTime.getMonth() + 1) +
...
let { limit, calcloanDate } = calcTermMonthAndLoanDate(
    "2023-08-28",
    "2023-10-27",
    12,
    true,
...
//2.根据计算出来的定期期数来控制 选择期限的数组

function getReList(limit) {
    if (limit === 6) {
        return;
    }
    if (limit < 6 && limit >= 3) {
